...
def pairwise_cosine_similarity(X):
    # 单位化每个特征向量
    X_norm = F.normalize(X, p=2, dim=1)  # shape: (N, D)
    # 使用矩阵乘法得到所有两两余弦相似度
    similarity_matrix = X_norm @ X_norm.T  # shape: (N, N)
    return similarity_matrix

def mean_off_diagonal(matrix: torch.Tensor):
    assert matrix.ndim == 2 and matrix.shape[0] == matrix.shape[1], "Input must be square matrix"
    N = matrix.shape[0]
    # 创建布尔掩码，True 表示保留非对角线元素
...
def measure_feature_change(model, image_paths, perturb_fn, path=True):
    ...
    avg_off_diag_pairwise_cos_sim = mean_off_diagonal(pairwise_cosine_similarity(perturbed_feats))
    return avg_off_diag_pairwise_cos_sim
